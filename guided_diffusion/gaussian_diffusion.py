...
class GaussianDiffusion:
    ...
    def p_sample_loop(
        ...
        **kwargs,
        ...
        if return_all:
            final["sample"] = final["sample"].clamp(-1.0, 1.0)
            return final
        else:
            return final["sample"].clamp(-1.0, 1.0)
    ...
    def p_sample_loop_progressive(
        ...
        **kwargs,
        ...
        if conf["repaint.schedule_jump_params"]:
            times = get_schedule_jump(**conf["repaint.schedule_jump_params"])
                ...
                if t_cur < t_last:  # reverse
                    with th.no_grad():
                        ...
                        if sample_dir is not None:
                            ...
                            save_grid(
                                normalize_image(image_before_step.clamp(-1, 1)),
                                os.path.join(
                                    sample_dir, "middles", f"mid-{t_last}.png"
                                ),
                            )
                            save_grid(
                                normalize_image(pred_x0),
                                os.path.join(
                                    sample_dir, "middles", f"pred-{t_last}.png"
                                ),
                            )
                        out = self.p_sample(
                            model,
